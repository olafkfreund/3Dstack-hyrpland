...
LayoutCalculator::LayoutCalculator(float perspective,
                                   float eyeDistance,
                                   float depthStep,
                                   float padding)
    : m_transform{perspective, eyeDistance},
    m_depthStep(depthStep),
    m_padding(padding) {
}

Vector2D Transform3D::project3DTo2D(Vector3D pos3D,
...
std::vector<WindowLayout> LayoutCalculator::calculateGridLayout(const std::vector<CWindow *> &windows) {
    ...
    return layouts;
} // LayoutCalculator::calculateGridLayout
...
std::vector<WindowLayout> LayoutCalculator::calculateCircularLayout(const std::vector<CWindow *> &windows) {
    ...
    float    radius = std::min(getWorkspaceSize().x,
                               getWorkspaceSize().y) * 0.3f;
    ...
    for (size_t i = 0; i < windows.size(); i++)
    {
        ...
        { 300, 200 };                      // Default window size for circular layout
    ...
    return layouts;
} // LayoutCalculator::calculateCircularLayout
...
std::vector<WindowLayout> LayoutCalculator::calculateSpiralLayout(const std::vector<CWindow *> &windows) {
    ...
    return layouts;
} // LayoutCalculator::calculateSpiralLayout
...
std::vector<WindowLayout> LayoutCalculator::calculateFibonacciLayout(const std::vector<CWindow *> &windows) {
    ...
    for (size_t i = 0; i < windows.size(); i++)
    {
        ...
        if (splitVertically)
        {
            ...
            layout.size =
            { currentSize.x * splitRatio, currentSize.y };
        ...
        else
        {
            float splitRatio = 1.0f / phi;
            layout.size =
            { currentSize.x, currentSize.y * splitRatio };
            layout.position = currentPos;
    ...
    return layouts;
} // LayoutCalculator::calculateFibonacciLayout
...
void LayoutCalculator::updateTransform(float perspective,
                                       float eyeDistance) {
