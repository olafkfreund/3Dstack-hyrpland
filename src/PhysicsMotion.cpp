...
PhysicsMotion::PhysicsMotion(float globalSpring, float globalDamping)
    : m_globalSpring(globalSpring), m_globalDamping(globalDamping) {}

void PhysicsMotion::addSpringForce(CWindow* window, Vector2D targetPos, Vector2D currentPos) {
...
void PhysicsMotion::addGravityForce(CWindow* window, Vector2D gravityCenter, float strength) {
    MotionState& motion = getOrCreateMotionState(window);
...
void PhysicsMotion::addRepulsionForce(CWindow* window, const std::vector<CWindow*>& otherWindows, float strength) {
    MotionState& motion = getOrCreateMotionState(window);
    ...
    for (CWindow* other : otherWindows) {
        ...
        Vector2D repulsionForce = {strength * 0.1f, strength * 0.1f};
        motion.acceleration = motion.acceleration + repulsionForce;
    }
}

Vector2D PhysicsMotion::getNextPosition(CWindow* window, Vector2D currentPos, float deltaTime) {
    MotionState& motion = getOrCreateMotionState(window);
    
    // Apply forces
    applyForces(motion, deltaTime);
...
float PhysicsMotion::getNextRotation(CWindow* window, float currentRotation, float deltaTime) {
    MotionState& motion = getOrCreateMotionState(window);
...
void PhysicsMotion::setMotionProperties(CWindow* window, float spring, float damping, float mass) {
    MotionState& motion = getOrCreateMotionState(window);
...
void PhysicsMotion::applyForces(MotionState& motion, float deltaTime) {
