...
AnimationSystem::AnimationSystem(float baseDuration,
                                 float maxStagger)
...
void AnimationSystem::startTransition(const std::vector<WindowLayout> &startLayouts,
    ...
    size_t numWindows = std::min(startLayouts.size(),
                                 endLayouts.size());
    ...
    m_isAnimating = true;
} // AnimationSystem::startTransition
...
void AnimationSystem::updateAnimation() {
    ...
    for (auto &transition : m_transitions)
    {
        ...
        currentLayout.position = lerp(transition.startPos,
                                      transition.endPos,
                                      easedProgress);
        currentLayout.size = lerp(transition.startSize,
                                  transition.endSize,
                                  easedProgress);
        currentLayout.rotation = lerp(transition.startRotation,
                                      transition.endRotation,
                                      easedProgress);
        currentLayout.scale = lerp(transition.startScale,
                                   transition.endScale,
                                   easedProgress);
        currentLayout.alpha = lerp(transition.startAlpha,
                                   transition.endAlpha,
                                   easedProgress);
        currentLayout.velocity = calculateVelocity(transition,
                                                   progress);

        // Apply the transformation to the window
        applyWindowTransform(transition.window,
                             currentLayout);
    }
...
{
    if (!m_isAnimating || m_transitions.empty())
    {
        return 1.0f;
    }
    ...
    return std::min(1.0f,
                    totalElapsed / (m_baseDuration + m_maxStagger));
}
...
void AnimationSystem::applyWindowTransform(CWindow *window,
...
Vector2D AnimationSystem::calculateVelocity(const WindowTransition &transition,
                                            float progress) {
    ...
    float futureProgress = std::min(1.0f,
                                    progress + dt / m_baseDuration);
    ...
    Vector2D currentPos = lerp(transition.startPos,
                               transition.endPos,
                               currentEased);
    Vector2D futurePos = lerp(transition.startPos,
                              transition.endPos,
                              futureEased);
