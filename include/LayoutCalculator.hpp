...
struct Transform3D
{
    ...
    Vector2D project3DTo2D(Vector3D pos3D,
...
{
    GRID, CIRCULAR, SPIRAL, FIBONACCI
};
...
public:
    explicit LayoutCalculator(float perspective = 800.0f,
                              float eyeDistance = 1000.0f,
                              float depthStep = 100.0f,
                              float padding = 20.0f);
    ...
    // Configuration
    void updateTransform(float perspective,
                         float eyeDistance);

    void setDepthStep(float depthStep) {
